...
export function MemoryStatePanel({ characterId, characterName, onConversationCleared, onStateChanged }: Props) {
  ...
  const refresh = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/memory/${characterId}?limit=24`);
      if (!res.ok) throw new Error("Failed to load companion memory");
  ...
  const handleSearch = useCallback(async () => {
    ...
    setSearching(true);
    try {
      const res = await fetch(`/api/memory/${characterId}/search?q=${encodeURIComponent(query.trim())}&limit=8`);
  ...
  const clearMemories = useCallback(async () => {
    ...
    try {
      await fetch(`/api/memory/${characterId}/clear`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ memory_type: null, reset_state: false }),
      });
      await refresh();
    } finally {
      setBusyAction(null);
  ...
  const resetState = useCallback(async () => {
    ...
    setBusyAction("state");
    try {
      await fetch(`/api/state/${characterId}/reset`, { method: "POST" });
      await refresh();
      onStateChanged?.();
    } finally {
      setBusyAction(null);
